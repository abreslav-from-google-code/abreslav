...
\begin{tabular}[c]{rcl}
	asdf & sdf adjhsad alsdf as alkj asdfasdl & sdfsfd\\
	\raisebox{-3pt}{$\frac{sd}{asda}$} & \hrulefill & \raisebox{-3pt}{dasd}\\
\end{tabular}
\tcgrule{x,e,e',s,t}{
    \premise{+[x : s]}{e' : s}
    \premise{-(:_1 = x), +[\mathtt{let\_binding}[x]](\langle 1 : [\forall] \rangle)}{e : t}
...
\tcgrule{x,bg,e,t}{
\premise{}{\mathtt{bind\_group}(bg)}
    \premise{-(:_1 = x), +[\mathtt{let\_binding}[x]](\langle ??? : [\forall] \rangle)}{e : t}
}{\mathbf{let}\;bg\;\mathbf{in}\;e : t}{}{}

The question marks indicate that by now we do not know what to extract from the premise. Indeed, what we did in the previous example was extracting a subproof which ended in a typing judgement $e' : s$, but here the premise is not a typing judgement, judgements are ``hidden'' in its proof:

\tcgrule{}{
\begin{array}{c}
	e'_1 : s_1\;\dots\;e'_n : s_n\\
	\vdots\\
	\vdash {\mathtt{bind\_group}(bg)}
\end{array}
